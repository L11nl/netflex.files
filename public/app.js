...
function generatorInboxTargets(email) {
  ...
  // مهم: مسار inbox9 الذي يفتحه المتصفح يعتمد أحياناً على جلسة/كوكيز،
  // لذلك القراءة الخلفية تبدأ بالمسار عديم الجلسة نفسه المستخدم في كود Python:
  // https://generator.email/DOMAIN/USERNAME
  // ثم نجرب رابط الصندوق المباشر الموثق، وأخيراً inbox9 كخطة احتياط.
  return [
    `${GENERATOR_BASE}/${domain}/${username}`,
    `${GENERATOR_BASE}/${address}`,
...
function isRequestedGeneratorMailboxHtml(html, email) {
  ...
  // لا نعتبر مجرد وجود جدول الرسائل نجاحاً؛ بعض البروكسيات قد تعيد الصفحة العامة.
  // يجب أن يكون عنوان صندوقنا ظاهراً أيضاً في HTML.
  const hasMailboxIdentity = Boolean(
    (address && lower.includes(address)) ||
    (username && domain && lower.includes(username) && lower.includes(domain))
  );
  if (!hasMailboxIdentity) return false;

  // عند وجود رسائل ستظهر هذه العناصر نفسها التي يعتمد عليها كود Python.
  if (/id=["']email-table["']|mess_bodiyy|subj_div_45g45gg|from_div_45g45gg/i.test(source)) return true;

  // الصندوق قد يكون فارغاً، لكن الصفحة ما زالت صحيحة إذا احتوت عنواننا.
  return hasMailboxIdentity;
}
...
async function fetchGeneratorHtml(email) {
  ...
  // نجرب كل تمثيلات الصندوق. الأهم هو /DOMAIN/USERNAME لأنه نفس المسار
  // الذي نجح في كود Python المرسل من المستخدم ولا يعتمد على فتح تبويب أو جلسة inbox9.
  for (const target of targets) {
    const freshTarget = `${target}?_=${Date.now()}-${Math.random().toString(16).slice(2)}`;
    candidates.push(freshTarget);
    for (const build of GENERATOR_CORS_BUILDERS) candidates.push(build(freshTarget));
  }
  ...
  for (const url of candidates) {
    try {
      ...
      lastError = new Error('تم الاتصال بالخدمة لكن الرد لم يكن صندوق هذا الإيميل.');
    } catch (error) {
      lastError = error;
    }
  }
...
function parseGeneratorMessages(html, email) {
  ...
  // بعض نسخ Generator.email تفصل جسم الرسالة عن جدول العناوين، لذلك
  // نبحث في الجدول أولاً ثم في كامل الصفحة كخطة احتياط.
  const root = table || doc;

  let senders = [...root.querySelectorAll('.from_div_45g45gg')];
  let subjects = [...root.querySelectorAll('.subj_div_45g45gg')];
  let bodies = [...root.querySelectorAll('.mess_bodiyy')];

  if (!senders.length) senders = [...doc.querySelectorAll('.from_div_45g45gg')];
  if (!subjects.length) subjects = [...doc.querySelectorAll('.subj_div_45g45gg')];
  if (!bodies.length) bodies = [...doc.querySelectorAll('.mess_bodiyy')];

  if (!senders.length && !subjects.length && !bodies.length) return [];
...
async function fetchGeneratorMessages(email, { retryEmpty = false } = {}) {
  const address = String(email?.address || '').trim().toLowerCase();
  if (!address || !address.endsWith(`@${GENERATOR_DOMAIN}`)) {
    throw new Error('هذا البريد لا يستخدم نطاق 5xu.vn.');
  }
  ...
  for (let attempt = 0; attempt < attempts; attempt += 1) {
    try {
      const response = await fetch(`/api/inbox?email=${encodeURIComponent(address)}&_=${Date.now()}`, {
        method: 'GET',
      ...
      if (!response.ok) throw new Error(data?.error || `فشل جلب صندوق البريد (${response.status}).`);

      const deleted = new Set(email.generatorDeletedIds || []);
      const seen = new Set(email.generatorSeenIds || []);
      const messages = (Array.isArray(data.messages) ? data.messages : [])
        .filter((message) => message?.id && !deleted.has(message.id))
        .map((message) => ({
          ...message,
          seen: seen.has(message.id) || Boolean(message.seen),
          verifications: Array.isArray(message.verifications) ? message.verifications : [],
          _generator: true
        }));

      if (messages.length || !retryEmpty || attempt === attempts - 1) return messages;
    } catch (error) {
      lastError = error;
  ...
  throw new Error(lastError?.message || 'تعذر الاتصال بخادم Railway لجلب الرسائل.');
}
